...
bool GuiMain::parse_dot_net_data(QTreeWidgetItem *item, DotNetOptionsTree *&out)
{
    ...
    auto hPipe = CreateNamedPipeW(pipe_name_full.c_str(), PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_NOWAIT,
                                  PIPE_UNLIMITED_INSTANCES, 0x1000, 0x10000, 0, nullptr);
    ...
    case QEvent::KeyPress:
    {
        ...
        else if (obj == ui.txt_pid)
        {
            ...
            if (keyEvent->key() >= Qt::Key_0 && keyEvent->key() <= Qt::Key_9 || keyEvent->key() == Qt::Key_Backspace ||
                keyEvent->key() == Qt::Key_Delete || keyEvent->key() == Qt::Key_Space)
            {
...
void GuiMain::inject_file()
{
    ...
    case 1:
        ...
        break;
    case 2:
        inj_data.Mode = INJECTION_MODE::IM_LdrpLoadDll;
        break;
    case 3:
        inj_data.Mode = INJECTION_MODE::IM_LdrpLoadDllInternal;
        break;
    case 4:
        inj_data.Mode = INJECTION_MODE::IM_ManualMap;
        break;
    default:
        inj_data.Mode = INJECTION_MODE::IM_LoadLibraryExW;
        break;
    }
    ...
    case 1:
        ...
        break;
    case 2:
        inj_data.Method = LAUNCH_METHOD::LM_SetWindowsHookEx;
        break;
    case 3:
        inj_data.Method = LAUNCH_METHOD::LM_QueueUserAPC;
        break;
    case 4:
        inj_data.Method = LAUNCH_METHOD::LM_KernelCallback;
        break;
    case 5:
        inj_data.Method = LAUNCH_METHOD::LM_FakeVEH;
        break;
    default:
        inj_data.Method = LAUNCH_METHOD::LM_NtCreateThreadEx;
        break;
    }
    ...
    for (; (*it) != Q_NULLPTR; ++it)
    {
        ...
        if (is_dot_net)
        {
            ...
            dot_net_items.push_back({fileStr.toStdWString(),
                                     {dot_net_options_list.at(0).toStdWString(), dot_net_options_list.at(1).toStdWString(),
                                      dot_net_options_list.at(2).toStdWString(), dot_net_arg.toStdWString()}}); // good code
        }
        else
        {
    ...
    for (const auto &i : dot_net_items)
    {
        ...
        {
            sprintf_s(buffer, ".NET injection (%d/%d) succeeded:\n  %ls = %p\n", dot_net_inj_count, (int)dot_net_items.size(), dll_name.c_str(),
                      dot_net_inj_data.hDllOut);
        }
...
void GuiMain::btn_generate_shortcut()
{
    ...
    auto silent = YesNoBox("Silent mode",
                           "Do you want the shortcut to be in silent mode?\nThis means that no console window will be spawned and\nthere will be no "
                           "notifications on the injection status.",
                           this);
    if (silent)
    {
        shortCut += L"-silent ";
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (YesNoBox("Success",
                     "The shortcut was created succesfully with the following name:\n" + fileName + "\n\nDo you want to open the shortcut location?",
                     this))
        {
            // stolen from here:
            // https://stackoverflow.com/questions/15300999/open-windows-explorer-directory-select-a-specific-file-in-delphi

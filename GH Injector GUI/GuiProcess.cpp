...
GuiProcess::GuiProcess(QWidget *parent) : QWidget(parent)
{
    ui.setupUi(this);
...
void GuiProcess::refresh_process()
{
    ...
    for (int i = 0; i < ui.tree_process->topLevelItemCount();)
    {
        ...
        auto search_list = [&pid](const ProcessData *entry) -> bool { return ((*entry) == pid); };
...
TreeWidgetItem::TreeWidgetItem(int type) : QTreeWidgetItem(type)
{
}

...
ProcessData::ProcessData(DWORD PID) : ProcessData()
{
    m_PID = PID;
...
ProcessData::ProcessData(const std::string &ExeName) : ProcessData()
{
...
ProcessData::ProcessData(const std::wstring &ExeName) : ProcessData()
{
    m_ExeName = ExeName;
...
{
    static auto sort_entries_pid = [](const PROCESSENTRY32W &lhs, const PROCESSENTRY32W &rhs) { return (lhs.th32ProcessID < rhs.th32ProcessID); };

    static auto sort_list_pid = [](const ProcessData *lhs, const ProcessData *rhs) { return ((*lhs) < (*rhs)); };
    ...
    {
        auto &i = *iter;
        auto search_entries = [&i](const ProcessData *entry) -> bool { return (i == (*entry)); };
...
bool SortProcessList(std::vector<ProcessData *> &list, SORT_SENSE sort)
{
    ...
    case SORT_SENSE::SS_PID_LO:
        std::sort(list.begin(), list.end(), [](const ProcessData *lhs, const ProcessData *rhs) { return ((*lhs) < (*rhs)); });
        break;

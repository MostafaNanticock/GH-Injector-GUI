...
bool update_injector(const std::wstring &newest_version, bool &ignore, InjectionLib *Lib)
{
    ...
    DownloadProgressWindow *UpdateWnd = new (std::nothrow)
        DownloadProgressWindow(QString("Downloading V") + QString::fromStdWString(newest_version), labels, "Initializing...", 250, Q_NULLPTR);
    ...
    else if (ret < 0)
    {
        ...
        if (ret == UPDATE_ERR_DELETE_FAIL || ret == UPDATE_ERR_RENAME_FAIL || ret == UPDATE_ERR_UNZIP_FAIL || ret == UPDATE_ERR_INCOMPLETE ||
            ret == UPDATE_ERR_LAUNCH_FAIL)
        {
